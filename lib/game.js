...
Game.prototype.load = function () {
  ...
  images = images.concat([Loader.loadImage('tiles', 'assets/tiles01.png')]);
...
Game.prototype.initMap = function () {
  ...
  /*Puerta niveles*/
    ...
    this.completedoor = false;

  var map = this.maps[this.currentMapIdx];

  map.addListener('complete-fast', function () {

    alert('Adelantaste una etapa');
    this.completedoor = true;
  }.bind(this));

  this.tileAtlas = Loader.getImage('tiles');

  const HeroConstructor = this.characters[this.currentCharacterIdx];
  this.hero = new HeroConstructor(map, 96, 96);
  this.camera = new Camera(map, 768, 448);
...
Game.prototype.tick = function (elapsed) {
  ...
  if (this.completed) {
    if (++this.currentMapIdx === this.maps.length) {
      return alert('Felicidades! Pasastes todas las etapas del Juego!');
    }
    this.initMap();
  }

  if (this.completedoor) {
    if (++this.currentMapIdx === this.maps.length) {
      return alert('WOW! Pasastes todas las etapas del Juego!');
    }
    this.initMap();
  }
